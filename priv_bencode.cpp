...
bool
bencode<Buffer>::e::value(Buffer &buffer, std::uint16_t in) noexcept {
  return encode_integer(buffer, "%hu", in);
} // bencode::e::value()

template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &buffer, std::int16_t in) noexcept {
  return encode_integer(buffer, "%h", in);
} // bencode::e::value()

template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &buffer, std::uint32_t in) noexcept {
  return encode_integer(buffer, "%u", in);
} // bencode::e::value()

template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &buffer, std::int32_t in) noexcept {
  return encode_integer(buffer, "%d", in);
} // bencode::e::value()

template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &buffer, std::uint64_t in) noexcept {
  return encode_integer(buffer, "%llu", in);
}

template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &buffer, std::int64_t in) noexcept {
  return encode_integer(buffer, "%lld", in);
}

template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &b, const char *str) noexcept {
  return value(b, str, std::strlen(str));
}

// template <typename Buffer>
// bool
// bencode<Buffer>::e::value(Buffer &b, const unsigned char *str) noexcept {
//   return value(b, str, std::strlen(str));
// }

template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &buffer, const char *str,
...
template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &b, const unsigned char *str,
                          std::size_t length) noexcept {
...
generic_encodePair(Buffer &buffer, const char *key, V val) {
  ...
  if (!bencode<Buffer>::e::value(buffer, key)) {
    return false;
  }

  if (!bencode<Buffer>::e::value(buffer, val)) {
    return false;
  }
...
template <typename Buffer>
bool
bencode<Buffer>::e::pair(Buffer &buffer, const char *key,
...
template <typename Buffer>
bool
bencode<Buffer>::e::pair(Buffer &buffer, const char *key, bool value) noexcept {
  return generic_encodePair(buffer, key, value);
}
/*----------------------------------------------------------*/
...
template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &b, const Contact &p) noexcept {
  return serialize(b, p);
} // bencode::e::value()

template <typename Buffer>
bool
bencode<Buffer>::e::pair(Buffer &buf, const char *key,
                         const Contact &p) noexcept {
  if (!bencode<Buffer>::e::value(buf, key)) {
    return false;
  }

  return bencode<Buffer>::e::value(buf, p);
} // bencode::e::pair()

template <typename Buffer>
bool
bencode<Buffer>::e::pair(Buffer &buf, const char *key, const byte *value,
  ...
  return bencode<Buffer>::e::value(buf, value, l);
}

template <typename Buffer>
bool
bencode<Buffer>::e::list(Buffer &buf, void *closure,
...
template <typename Buffer>
bool
bencode<Buffer>::e::pair_compact(Buffer &buf, const char *key,
...
template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &b, std::size_t length, void *closure,
                          bool (*f)(Buffer &, void *)) noexcept {
...
xxx(Buffer &buf, const char *key, const dht::Node **list,
  ...
  return bencode<Buffer>::e::value(buf, len, &arg, [](auto &b, void *a) {
...
template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &buf, const dht::Node &node) noexcept {
  return bencode<Buffer>::e::dict(buf, [&node](Buffer &b) { //

    if (!bencode<Buffer>::e::pair(b, "id", node.id.id, sizeof(node.id.id))) {
      return false;
    }

    if (!bencode<Buffer>::e::pair(b, "good", node.good)) {
      return false;
    }

    if (!bencode<Buffer>::e::pair(b, "ping_outstanding",
...
template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &buf, const dht::Bucket &t) noexcept {
  return bencode<Buffer>::e::list(buf, (void *)&t, [](Buffer &b, void *arg) { //
...
template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &buf, const dht::RoutingTable &t) noexcept {
  // used by dump

  return bencode<Buffer>::e::dict(buf, [&t](Buffer &b) {
    // dht::Infohash id; // TODO
    // if (!pair(b, "id", id.id, sizeof(id.id))) {
    //   return false;
    // }

    if (!bencode<Buffer>::e::value(b, "bucket")) {
      return false;
    }

    if (!bencode<Buffer>::e::value(b, t.bucket)) {
      return false;
    }
...
template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &buf, const dht::Peer &t) noexcept {
  // used by dump

  return bencode<Buffer>::e::dict(buf, [&t](Buffer &b) { //
    if (!bencode<Buffer>::e::value(b, "contact")) {
      return false;
    }
    if (!bencode<Buffer>::e::value(b, t.contact)) {
      return false;
    }
...
template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &buf, const dht::KeyValue &t) noexcept {
  // used by dump
  return bencode<Buffer>::e::dict(buf, [&t](Buffer &b) { //
    if (!bencode<Buffer>::e::pair(b, "id", t.id.id, sizeof(t.id.id))) {
      return false;
    }

    if (!bencode<Buffer>::e::value(b, "list")) {
      return false;
    }

    return bencode<Buffer>::e::list(
...
bencode<Buffer>::e::pair(Buffer &buf, const char *key,
  ...
  return dict(buf, [&t](Buffer &b) {
    if (!bencode<Buffer>::e::pair(b, "ping", t.ping)) {
      return false;
    }
    if (!bencode<Buffer>::e::pair(b, "find_node", t.find_node)) {
      return false;
    }
    if (!bencode<Buffer>::e::pair(b, "get_peers", t.get_peers)) {
      return false;
    }
    if (!bencode<Buffer>::e::pair(b, "announce_peer", t.announce_peer)) {
      return false;
    }
    if (!bencode<Buffer>::e::pair(b, "error", t.error)) {
      return false;
    }
...
bencode<Buffer>::e::pair(Buffer &buf, const char *key,
  ...
  return dict(buf, [&d](Buffer &b) {
    if (!bencode<Buffer>::e::pair(b, "request", d.request)) {
      return false;
    }

    if (!bencode<Buffer>::e::pair(b, "response_timeout", d.response_timeout)) {
      return false;
    }

    if (!bencode<Buffer>::e::pair(b, "response", d.response)) {
      return false;
    }

    if (!bencode<Buffer>::e::pair(b, "parse_error", d.parse_error)) {
      return false;
    }
    // TODO
    return true;
...
template <typename Buffer>
bool
bencode<Buffer>::e::value(Buffer &buf, const sp::list<Contact> &t) noexcept {
  // used by dump
...
template struct bencode<sp::CircularByteBuffer>;
template struct bencode<sp::Sink>;
} // namespace sp
